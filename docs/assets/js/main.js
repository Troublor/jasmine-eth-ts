!(function () {
    ...
    (e.FieldRef.prototype.toString = function () {
        return (
            null == this._stringValue && (this._stringValue = this.fieldName + e.FieldRef.joiner + this.docRef),
            this._stringValue
        );
    }),
    (e.Set = function (e) {
        ...
        var e = {
            ...
            I = function (r) {
                ...
                if (
                    ...
                    (a = y),
                    (o = p).test(r) ? (r = r.replace(o, "$1$2")) : a.test(r) && (r = r.replace(a, "$1$2")),
                    (a = v),
                    (o = m).test(r))
                ) {
                    var I = o.exec(r);
        ...
        for (var i = new e.TokenSet(), n = [{ node: i, editsRemaining: r, str: t }]; n.length; ) {
            ...
            if (0 != s.editsRemaining) {
                ...
                if (
                    ...
                    1 < s.str.length &&
                            n.push({ node: s.node, editsRemaining: s.editsRemaining - 1, str: s.str.slice(1) }),
            ...
            for (var f = 0; f < c.length; f++) {
                ...
                for (var v = 0; v < this.metadataWhitelist.length; v++) {
                    var g = this.metadataWhitelist[v],
                        x = p.metadata[g];
                    null == this.invertedIndex[p][o][i][g] && (this.invertedIndex[p][o][i][g] = []),
    ...
    (e.Builder.prototype.createFieldVectors = function () {
        for (
            ...
            for (
                ...
                (v =
                            (m * ((this._k1 + 1) * w)) /
                            (this._k1 * (1 - this._b + this._b * (u / this.averageFieldLength[a])) + w)),
                (v *= f),
                (v *= p),
    ...
    (e.QueryLexer.lexTerm = function (t) {
        if ((1 < t.width() && (t.backup(), t.emit(e.QueryLexer.TERM)), t.ignore(), t.more()))
    ...
    (e.QueryLexer.lexText = function (t) {
        for (;;) {
            ...
            if (92 != r.charCodeAt(0)) {
                ...
                if ("~" == r)
                    return t.backup(), 0 < t.width() && t.emit(e.QueryLexer.TERM), e.QueryLexer.lexEditDistance;
                    ...
                    (c.namespace != namespace && "*" != c.namespace) ||
                            context.querySelectorAll(c.selector).forEach(function (el) {
                                el.dataset.hasInstance ||
                                    (new c.constructor({ el: el }), (el.dataset.hasInstance = String(!0)));
                            });
                });
            }),
            Application
    ...
    var EventTarget = (function () {
        ...
        return (
            ...
            (EventTarget.prototype.dispatchEvent = function (event) {
                ...
                for (var stack = this.listeners[event.type].slice(), i = 0, l = stack.length; i < l; i++)
...
(function (typedoc) {
    typedoc.throttle = function (fn, wait) {
        ...
        return function () {
            ...
            time + wait - Date.now() < 0 && (fn.apply(void 0, args), (time = Date.now()));
        };
    };
})(typedoc || (typedoc = {})),
(function (typedoc) {
    var Viewport = (function (_super) {
        ...
        return (
            ...
            (Viewport.prototype.triggerResize = function () {
                var event = new CustomEvent("resize", { detail: { width: this.width, height: this.height } });
            ...
            (Viewport.prototype.onScroll = function () {
                ...
                var event = new CustomEvent("scroll", { detail: { scrollTop: this.scrollTop } });
        ...
        FilterItemSelect = (function (_super) {
            ...
            return (
                    ...
                    if (select) {
                        ...
                        this.select.querySelectorAll("li").forEach(function (el) {
                            el.addEventListener(typedoc.pointerUp, function (e) {
                                select.classList.remove("active"), _this.setValue(e.target.dataset.value || "");
                            });
                        }),
                        document.addEventListener(typedoc.pointerDown, function (e) {
                            _this.select.contains(e.target) || _this.select.classList.remove("active");
                        });
                    }
                }),
                (FilterItemSelect.prototype.handleValueChange = function (oldValue, newValue) {
                    ...
                    selected &&
                            label &&
                            (selected.classList.add("selected"), (label.textContent = selected.textContent)),
    ...
    var MenuHighlight = (function (_super) {
        ...
        return (
                ...
                this.el.querySelectorAll("a").forEach(function (el) {
                    ...
                    if (-1 != href.indexOf("#") && href.substr(0, base.length) == base) {
                        var hash = href.substr(href.indexOf("#") + 1),
                            anchor = document.querySelector("a.tsd-anchor[name=" + hash + "]"),
                            link = el.parentNode;
                        anchor && link && _this.anchors.push({ link: link, anchor: anchor, position: 0 });
                    }
                }),
                this.onResize();
            }),
            (MenuHighlight.prototype.onResize = function () {
                for (var anchor, index = 0, count = this.anchors.length; index < count; index++) {
                    var rect = (anchor = this.anchors[index]).anchor.getBoundingClientRect();
                    anchor.position = rect.top + document.body.scrollTop;
                }
                this.anchors.sort(function (a, b) {
                    return a.position - b.position;
                });
                var event = new CustomEvent("scroll", { detail: { scrollTop: typedoc.viewport.scrollTop } });
                this.onScroll(event);
            }),
            (MenuHighlight.prototype.onScroll = function (event) {
                ...
                this.index != index &&
                        (-1 < this.index && this.anchors[this.index].link.classList.remove("focus"),
                        (this.index = index),
                        -1 < this.index && this.anchors[this.index].link.classList.add("focus"));
            }),
            MenuHighlight
        );
    })(typedoc.Component);
    ...
    !(function (search) {
        ...
        var Search = (function (_super) {
            function Search(options) {
                ...
                if (!field || !results) throw new Error("The input field or the result list wrapper are not found");
                return (
                    (_this.field = field),
            ...
            return (
                        ...
                        url
                            ? fetch(url)
                                .then(function (response) {
                                    if (!response.ok) throw new Error("The search index is missing");
                                ...
                                .catch(function (error) {
                                    console.error(error), _this.setLoadingState(SearchLoadingState.Failure);
                                })
                ...
                (Search.prototype.updateResults = function () {
                    if (
                        ...
                        0 === res.length && (res = this.index.search("*" + this.query + "~1*"));
                        for (var i = 0, c = Math.min(10, res.length); i < c; i++) {
                            var row = this.data.rows[Number(res[i].ref)],
                                name = row.name.replace(new RegExp(this.query, "i"), function (match) {
                                    return "<b>" + match + "</b>";
                                }),
                                parent = row.parent || "";
                            (parent = parent.replace(new RegExp(this.query, "i"), function (match) {
                                return "<b>" + match + "</b>";
                ...
                (Search.prototype.setHasFocus = function (value) {
                    this.hasFocus != value &&
                            ...
                            value ? (this.setQuery(""), (this.field.value = "")) : (this.field.value = this.query));
                }),
                ...
                (Search.prototype.setCurrentResult = function (dir) {
                    ...
                    if (current) {
                        var rel = 1 == dir ? current.nextElementSibling : current.previousElementSibling;
                        rel && (current.classList.remove("current"), rel.classList.add("current"));
                    } else
                        (current = this.results.querySelector(1 == dir ? "li:first-child" : "li:last-child")) &&
                                current.classList.add("current");
                }),
                (Search.prototype.gotoCurrentResult = function () {
                    var current = this.results.querySelector(".current");
                    if ((current || (current = this.results.querySelector("li:first-child")), current)) {
                ...
                (Search.prototype.bindEvents = function () {
                    ...
                    document.body.addEventListener("keydown", function (e) {
                        e.altKey ||
                                    e.ctrlKey ||
                                    e.metaKey ||
                                    (!_this.hasFocus && 47 < e.keyCode && e.keyCode < 112 && _this.field.focus());
                    });
                }),
                Search
            );
        })(typedoc.Component);
        (search.Search = Search), typedoc.registerComponent(Search, "#tsd-search");
    ...
    var SignatureGroup = (function () {
            ...
            return (
                (SignatureGroup.prototype.addClass = function (className) {
                    return this.signature.classList.add(className), this.description.classList.add(className), this;
                }),
    ...
    var Toggle = (function (_super) {
        ...
        return (
            ...
            (Toggle.prototype.onDocumentPointerUp = function (e) {
                var _this = this;
                if (!typedoc.hasPointerMoved && this.active && e.target.closest(".col-menu")) {
